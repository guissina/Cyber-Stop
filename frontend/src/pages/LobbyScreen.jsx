...
export default function LobbyScreen() {
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      // Exibe erro se a criação falhar
      alert(e.response?.data?.error || e.message);
    } finally {
      // Garante que o estado de 'criando' seja resetado
      setCreating(false);
    }
  };
  ...
  const joinExisting = async () => {
    ...
    try {
      ...
      // (Nota: Se a sala já estiver em jogo, a WaitingRoomScreen deve redirecionar automaticamente para GameScreen)
      nav(`/waiting/${data.sala_id}`);
    } catch (e) {
      // Exibe erro se não conseguir entrar
      alert(e.response?.data?.error || e.message);
    } finally {
      // Garante que o estado de 'entrando' seja resetado
      setJoining(false);
    }
  };
